...
本文描述 RubiKey 将智能魔方支持范围从单一 GAN 扩展到 Moyu32 / 魔域新协议时的第一阶段实施方案，并补充当前已落地的 V10 AI 陀螺仪增强。
...
- 对支持陀螺仪的 Moyu32 设备，允许在不影响正常转动映射的前提下，隐式启用“姿态 -> 鼠标移动”
...
- 对 V10 AI 这类带陀螺仪数据流的设备，连接后可请求开启 gyro
- 陀螺仪鼠标不替代原有 move 宏，而是与正常转动并存
- 控制模型为：
  - 中立姿态校准
  - 超出阈值后持续移动
  - 回到阈值内后停止
...
虽然 V10 AI 四元数已经按参考协议完成解码，但设备坐标系与屏幕上下左右之间的映射仍需要真机确认。
